...
function useRouter() {
  ...
  const navigate = (page, slug) => {
    ...
    else if (page === "post") path = `/blog/${slug}`;
    else if (page === "studio") path = "/studio";
    ...
    <>
      <style>{`
        ...
        .mobile-nav-link {
          ...
          transition: color 0.2s; display: block; cursor: pointer;
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 500, padding: "0 5vw", height: "70px", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled || menuOpen ? "rgba(5,5,8,0.95)" : "transparent", backdropFilter: scrolled || menuOpen ? "blur(16px)" : "none", borderBottom: scrolled || menuOpen ? "1px solid rgba(255,255,255,0.05)" : "none", transition: "all 0.4s ease" }}>
        <a href="/" onClick={(e) => { e.preventDefault(); navigate("home"); }} style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "1.2rem", color: "var(--white)", textDecoration: "none", letterSpacing: "0.02em" }}>
  ...
  return (
    <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", padding: "0 5vw", paddingTop: "70px", paddingBottom: "2rem", position: "relative", overflow: "hidden" }}>
      <style>{`
        ...
        @keyframes mentorSlideIn { from { opacity: 0; transform: translateX(-50px); } to { opacity: 1; transform: translateX(0); } }
        @keyframes shimmerMove { 0% { left: -60%; } 100% { left: 120%; } }
        @keyframes heroFadeUp { from { opacity: 0; transform: translateY(24px); } to { opacity: 1; transform: translateY(0); } }
        .hero-outer { width: 100%; max-width: 1200px; margin: 0 auto; display: grid; grid-template-columns: 1fr 1fr; gap: 2.5rem; align-items: center; }
        .hero-left { display: flex; flex-direction: column; gap: 1.4rem; }
        .hero-right { display: flex; flex-direction: column; gap: 1.2rem; }
        .hero-pill { display: inline-flex; align-items: center; gap: 0.5rem; background: rgba(255,92,26,0.08); border: 1px solid rgba(255,92,26,0.25); border-radius: 100px; padding: 0.5rem 1.1rem; width: fit-content; animation: heroFadeUp 0.6s ease 0.1s both; }
        .hero-pill-dot { width: 7px; height: 7px; border-radius: 50%; background: var(--orange); box-shadow: 0 0 8px var(--orange); flex-shrink: 0; }
        .hero-pill-text { font-size: 0.72rem; font-weight: 600; letter-spacing: 0.07em; text-transform: uppercase; color: var(--orange); }
        .hero-headline { font-family: var(--font-display); font-weight: 800; font-size: clamp(3rem, 6vw, 5.5rem); line-height: 0.88; letter-spacing: -0.03em; margin: 0; animation: heroFadeUp 0.7s ease 0.2s both; }
        .hero-headline .line-orange { color: var(--orange); text-shadow: 0 0 80px rgba(255,92,26,0.35); display: block; }
        .hero-headline .line-white { color: var(--white); display: block; }
        .hero-usp-row { display: flex; flex-direction: column; gap: 0.5rem; animation: heroFadeUp 0.7s ease 0.35s both; }
        .hero-usp-item { display: flex; align-items: center; gap: 0.6rem; font-size: 0.88rem; color: rgba(240,237,232,0.75); font-weight: 500; }
        .hero-usp-item::before { content: ''; width: 5px; height: 5px; border-radius: 50%; background: var(--orange); flex-shrink: 0; }
        .hero-cta { display: inline-flex; align-items: center; gap: 0.5rem; background: var(--orange); color: white; padding: 0.9rem 2rem; border-radius: 8px; text-decoration: none; font-weight: 700; font-size: 0.9rem; letter-spacing: 0.06em; text-transform: uppercase; box-shadow: 0 0 40px var(--orange-glow); transition: transform 0.2s, box-shadow 0.2s; width: fit-content; animation: heroFadeUp 0.7s ease 0.5s both; }
        .hero-cta:hover { transform: translateY(-3px); box-shadow: 0 10px 50px var(--orange-glow); }
        .mentor-card { display: flex; flex-direction: row; background: rgba(10,10,16,0.95); border: 1px solid rgba(255,193,7,0.2); border-radius: 20px; overflow: hidden; animation: mentorSlideIn 0.7s cubic-bezier(0.16,1,0.3,1) 0.4s both; transition: box-shadow 0.3s, border-color 0.3s; }
        .mentor-card:hover { box-shadow: 0 24px 64px rgba(255,193,7,0.12); border-color: rgba(255,193,7,0.4); }
        .mentor-photo-top { width: 160px; flex-shrink: 0; position: relative; overflow: hidden; border-radius: 0; min-height: 220px; }
        .mentor-photo-top img { width: 100%; height: 100%; object-fit: cover; object-position: center 25%; display: block; }
        .mentor-photo-top::after { content: ''; position: absolute; inset: 0; background: linear-gradient(to right, transparent 60%, rgba(10,10,16,0.6)); pointer-events: none; }
        .online-dot { position: absolute; bottom: 10px; right: 10px; width: 12px; height: 12px; background: #25D366; border-radius: 50%; border: 2px solid rgba(10,10,16,1); z-index: 2; animation: pulse-dot 2s ease-in-out infinite; }
        .mentor-info { flex: 1; display: flex; flex-direction: column; gap: 0.55rem; padding: 1.2rem 1.2rem 1.2rem 1rem; min-width: 0; }
        .mentor-name-row { display: flex; align-items: center; gap: 0.45rem; flex-wrap: wrap; }
        .mentor-name { font-family: var(--font-display); font-weight: 800; font-size: 1.05rem; color: var(--white); }
        .mentor-yrs-badge { background: rgba(255,193,7,0.12); border: 1px solid rgba(255,193,7,0.35); border-radius: 100px; padding: 0.15rem 0.6rem; font-size: 0.65rem; font-weight: 700; color: var(--orange); letter-spacing: 0.05em; white-space: nowrap; }
        .mentor-role { font-size: 0.68rem; color: rgba(240,237,232,0.35); font-weight: 500; letter-spacing: 0.05em; text-transform: uppercase; margin-top: -0.2rem; }
        .mentor-divider { height: 1px; background: rgba(255,255,255,0.06); }
        .agency-row { display: flex; flex-wrap: wrap; gap: 0.28rem; align-items: center; }
        .agency-label { font-size: 0.58rem; color: rgba(240,237,232,0.28); font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; width: 100%; margin-bottom: 0.1rem; }
        .agency-badge { font-size: 0.62rem; font-weight: 700; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.09); border-radius: 100px; padding: 0.15rem 0.48rem; color: rgba(240,237,232,0.6); letter-spacing: 0.04em; transition: all 0.2s; cursor: default; white-space: nowrap; }
        .agency-badge:hover { background: rgba(255,193,7,0.1); border-color: rgba(255,193,7,0.35); color: var(--orange); }
        .campaign-stat { display: inline-flex; align-items: center; gap: 0.35rem; font-size: 0.72rem; font-weight: 800; color: var(--orange); background: rgba(255,193,7,0.07); border: 1px solid rgba(255,193,7,0.2); border-radius: 8px; padding: 0.28rem 0.75rem; width: fit-content; }
        .industry-tag { font-size: 0.62rem; font-weight: 600; background: rgba(255,92,26,0.08); border: 1px solid rgba(255,92,26,0.22); border-radius: 100px; padding: 0.15rem 0.48rem; color: rgba(240,237,232,0.75); transition: all 0.2s; white-space: nowrap; }
        .industry-tag:hover { background: rgba(255,92,26,0.18); color: var(--white); }
        @media (max-width: 600px) { .mentor-card { flex-direction: column; } .mentor-photo-top { width: 100%; height: 260px; } .mentor-photo-top img { object-fit: cover; object-position: center center; } .mentor-photo-top::after { background: linear-gradient(to bottom, transparent 50%, rgba(10,10,16,0.7)); } .mentor-info { padding: 1.1rem; gap: 0.5rem; } .mentor-name { font-size: 1rem; } }
        .hero-badge-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0.85rem; animation: heroFadeUp 0.7s ease 0.3s both; }
        .hero-stat-card { background: var(--surface); border: 1px solid var(--border); border-radius: 14px; padding: 1.2rem 1rem; transition: border-color 0.3s, transform 0.3s, box-shadow 0.3s; }
        .hero-stat-card:hover { border-color: rgba(255,92,26,0.4); transform: translateY(-3px); box-shadow: 0 16px 40px rgba(255,92,26,0.1); }
        @media (max-width: 768px) { .hero-outer { grid-template-columns: 1fr; gap: 2rem; } .hero-headline { font-size: clamp(2.8rem, 10vw, 4rem); } .hero-right { flex-direction: column; } .mentor-card { max-width: 100%; } .hero-badge-grid { gap: 0.7rem; } .hero-stat-card { padding: 1rem 0.85rem; } }
      `}</style>
      ...
      <div className="hero-outer">
        <div className="hero-left">
          ...
          <h1 className="hero-headline">
            ...
            <span style={{ display: "block", fontSize: "clamp(1rem, 2vw, 1.6rem)", fontWeight: 600, color: "rgba(240,237,232,0.45)", letterSpacing: "0.02em", marginTop: "0.5rem", fontFamily: "var(--font-body)" }}>Training that gets you hired.</span>
          </h1>
          <div className="hero-usp-row">
            {["Build job-ready skills companies actually hire for", "Run & scale your own digital marketing campaigns", "Learn from 12+ years of real agency experience"].map((t, i) => (
              <div key={i} className="hero-usp-item">{t}</div>
            ))}
          </div>
          <a href="https://wa.me/919311048234" target="_blank" rel="noreferrer" className="hero-cta">Start Learning →</a>
        </div>
          ...
          <div className="mentor-card">
            ...
            <div className="mentor-info">
              ...
              <div className="agency-row">
                ...
                {["WPP", "IPG", "Publicis", "Omnicom", "Havas"].map((a) => (<span key={a} className="agency-badge">{a}</span>))}
              </div>
              <span className="campaign-stat">💰 Handled ₹100 Cr+ Campaigns</span>
              <div className="mentor-divider" />
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.28rem" }}>
                {["Automobile", "Telecom", "FMCG", "Ecommerce"].map((tag) => (<span key={tag} className="industry-tag">{tag}</span>))}
              </div>
            </div>
          </div>
          <div className="hero-badge-grid">
            {[{ icon: "📅", title: "2 +1 Months", sub: "Course + Internship" }, { icon: "🎯", title: "Live Classes", sub: "Real-time learning" }, { icon: "💼", title: "100%", sub: "Job Assistance" }, { icon: "🤖", title: "AI Tools", sub: "Included in curriculum" }].map((s, i) => (
    ...
    <section id="program" style={{ padding: "5rem 5vw", background: "var(--surface)" }}>
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        <div className="reveal" style={{ marginBottom: "4rem" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(2.2rem, 4vw, 4rem)", lineHeight: 0.95 }}>Digital Marketing is<br /><span style={{ color: "var(--orange)" }}>everywhere. Are you?</span></h2>
        </div>
        <style>{`
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "1.5rem" }}
          ref={(el) => {
            ...
            const obs = new IntersectionObserver((entries) => { entries.forEach((e) => { if (e.isIntersecting) { const idx = [...cards].indexOf(e.target); setTimeout(() => e.target.classList.add("card-visible"), idx * 150); } }); }, { threshold: 0.15 });
          ...
          {[
            ...
            <div key={i} className="for-card" onMouseEnter={(e) => { e.currentTarget.style.borderColor = c.color + "66"; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = "var(--border)"; }}>
...
function Curriculum() {
  ...
  return (
    <section style={{ padding: "clamp(4rem, 8vw, 8rem) 5vw", background: "var(--bg)" }}>
      <style>{`@keyframes itemIn { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } } .curr-item { animation: itemIn 0.4s cubic-bezier(0.16,1,0.3,1) both; }`}</style>
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <div key={animKey} style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(100%, 280px), 1fr))", gap: "1rem" }}>
          {tracks[active].items.map((item, i) => (
            <div key={i} className="curr-item" style={{ display: "flex", alignItems: "center", gap: "1rem", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "10px", padding: "1.1rem 1.4rem", transition: "border-color 0.25s, transform 0.25s, box-shadow 0.25s", animationDelay: `${i * 0.06}s` }}
...
function ToolsMarquee() {
  ...
  return (
    <section style={{ padding: "2.5rem 0", background: "var(--bg)", overflow: "hidden" }}>
      <style>{`.tools-track { display: flex; align-items: center; animation: marquee 32s linear infinite; width: max-content; } .tools-track:hover { animation-play-state: paused; } .tool-card { display: flex; flex-direction: column; align-items: center; gap: 0.55rem; margin: 0 0.8rem; cursor: default; } .tool-icon-wrap { width: 64px; height: 64px; border-radius: 16px; display: flex; align-items: center; justify-content: center; border: 1px solid rgba(255,255,255,0.08); transition: transform 0.3s cubic-bezier(0.34,1.56,0.64,1), box-shadow 0.3s; padding: 10px; background: white; } .tool-card:hover .tool-icon-wrap { transform: translateY(-5px) scale(1.08); } .tool-icon-wrap img { width: 100%; height: 100%; object-fit: contain; display: block; } .tool-card-name { font-size: 0.6rem; font-weight: 700; color: rgba(240,237,232,0.4); letter-spacing: 0.06em; text-transform: uppercase; transition: color 0.3s; white-space: nowrap; } .tool-card:hover .tool-card-name { color: rgba(240,237,232,0.85); } @media (max-width: 600px) { .tool-icon-wrap { width: 50px; height: 50px; border-radius: 12px; padding: 8px; } .tool-card-name { font-size: 0.52rem; } .tool-card { margin: 0 0.55rem; gap: 0.4rem; } }`}</style>
      <div style={{ textAlign: "center", marginBottom: "1.8rem" }}>
        <span style={{ fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--orange)" }}>Platforms & Tools You Will Master</span>
      </div>
        ...
        <div className="tools-track">
          {all.map((tool, i) => (
            <div key={i} className="tool-card">
              <div className="tool-icon-wrap" style={{ boxShadow: `0 4px 20px rgba(${tool.color},0.15)` }} onMouseEnter={(e) => { e.currentTarget.style.boxShadow = `0 8px 32px rgba(${tool.color},0.4)`; }} onMouseLeave={(e) => { e.currentTarget.style.boxShadow = `0 4px 20px rgba(${tool.color},0.15)`; }}>
...
/* ─── COMPARISON ─────────────────────────────────────────────── */
function Comparison() {
  const rows = [["Practical, hands-on live training", "Mostly theory-based"], ["Live platform with new-age AI tools", "Recorded / outdated modules"], ["12+ years real agency experience", "Limited agency exposure"], ["Industry-aligned, always updated", "Generic syllabus"], ["Real campaign breakdowns", "Limited real exposure"], ["Structured job prep & interview training", "Empty placement promises"]];
  return (
    <section id="results" style={{ padding: "clamp(4rem, 8vw, 8rem) 5vw", background: "var(--surface)" }}>
      <style>{`.split-table { width: 100%; border-collapse: separate; border-spacing: 0; border-radius: 16px; overflow: hidden; border: 1px solid var(--border); } .split-table thead tr th { padding: 1rem 1.4rem; font-size: 0.72rem; font-weight: 800; letter-spacing: 0.14em; text-transform: uppercase; } .split-table thead tr th:first-child { background: rgba(255,92,26,0.12); color: var(--orange); border-right: 1px solid var(--border); width: 50%; } .split-table thead tr th:last-child { background: rgba(255,255,255,0.03); color: rgba(240,237,232,0.35); width: 50%; } .split-row td { padding: 0.95rem 1.4rem; font-size: 0.9rem; font-weight: 500; line-height: 1.5; border-top: 1px solid rgba(255,255,255,0.05); vertical-align: middle; } .split-row td:first-child { color: var(--white); background: rgba(255,92,26,0.04); border-right: 1px solid var(--border); } .split-row td:last-child { color: rgba(240,237,232,0.3); background: transparent; } .split-row:hover td:first-child { background: rgba(255,92,26,0.09); } @media (max-width: 540px) { .split-table thead tr th, .split-row td { padding: 0.8rem 0.9rem; font-size: 0.82rem; } }`}</style>
      <div style={{ maxWidth: "860px", margin: "0 auto" }}>
        <div className="reveal" style={{ marginBottom: "3.5rem" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(2.2rem, 4vw, 4rem)", lineHeight: 0.95 }}>What makes us<br /><span style={{ color: "var(--orange)" }}>different</span></h2>
        </div>
        <div className="reveal" style={{ background: "var(--bg)", borderRadius: "16px", overflow: "hidden" }}>
          <table className="split-table">
            <thead><tr><th>DigitalWithChirag</th><th>Others</th></tr></thead>
            <tbody>{rows.map(([us, them], i) => (<tr key={i} className="split-row"><td>{us}</td><td>{them}</td></tr>))}</tbody>
          </table>
        </div>
      </div>
    </section>
...
/* ─── OUTCOMES ───────────────────────────────────────────────── */
function Outcomes() {
  const outcomes = [{ icon: "🚀", text: "Launch Google & Meta ad campaigns" }, { icon: "⚙️", text: "Optimize Ad Campaigns Confidently" }, { icon: "💡", text: "Crack digital marketing interviews" }, { icon: "📊", text: "Plan Budgets like an agency professional" }, { icon: "📈", text: "Create Reports and Generate Insights" }, { icon: "🏪", text: "Run digital marketing for your business" }];
  return (
    <section style={{ padding: "5rem 5vw", background: "var(--bg)" }}>
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        <div className="reveal" style={{ marginBottom: "4rem" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(2.2rem, 4vw, 4rem)", lineHeight: 0.95 }}>What You'll Be Able To<br /><span style={{ color: "var(--orange)" }}>Do After This Program</span></h2>
        </div>
        <div className="stagger-children" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "1.2rem" }}>
          {outcomes.map((o, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: "1.2rem", padding: "1.5rem", background: "var(--surface)", borderRadius: "12px", border: "1px solid var(--border)", transition: "transform 0.3s, box-shadow 0.3s, border-color 0.3s" }}
...
function AboutCarousel() {
  ...
  useEffect(() => { timerRef.current = setInterval(() => { setCurrent((c) => (c + 1) % 3); }, 3000); return () => clearInterval(timerRef.current); }, []);

  return (
    <div style={{ position: "relative", width: "100%" }}>
      <style>{`.about-carousel-img { opacity: 0; transform: scale(1.04); transition: opacity 0.5s ease, transform 0.5s ease; position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; object-position: top center; pointer-events: none; } .about-carousel-img.active { opacity: 1; transform: scale(1); z-index: 2; } .about-carousel-img.inactive { opacity: 0; z-index: 1; }`}</style>
      <div style={{ position: "relative", width: "100%", maxWidth: "320px", margin: "0 auto" }}>
        <div style={{ position: "relative", aspectRatio: "3/4", borderRadius: "32px", overflow: "hidden", border: "2px solid var(--border)", boxShadow: "0 0 60px rgba(255,193,7,0.15), 0 40px 80px rgba(0,0,0,0.5)", background: "var(--bg)" }}>
          {slides.map((s, i) => (<img key={i} src={s.image} alt={`Chirag Atreja ${i + 1}`} className={`about-carousel-img${i === current ? " active" : " inactive"}`} />))}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem", marginTop: "2.5rem" }}>
          <button onClick={prev} style={{ width: "36px", height: "36px", borderRadius: "50%", background: "var(--bg)", border: "1px solid var(--border)", color: "var(--white)", fontSize: "1rem", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "border-color 0.2s, background 0.2s" }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--orange)"; e.currentTarget.style.background = "rgba(255,193,7,0.1)"; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = "var(--border)"; e.currentTarget.style.background = "var(--bg)"; }}>←</button>
          <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
            {slides.map((_, i) => (<button key={i} onClick={() => goTo(i)} style={{ width: i === current ? "24px" : "8px", height: "8px", borderRadius: "100px", background: i === current ? "var(--orange)" : "rgba(255,255,255,0.2)", border: "none", cursor: "pointer", padding: 0, transition: "all 0.3s ease" }} />))}
          </div>
          <button onClick={next} style={{ width: "36px", height: "36px", borderRadius: "50%", background: "var(--bg)", border: "1px solid var(--border)", color: "var(--white)", fontSize: "1rem", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "border-color 0.2s, background 0.2s" }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--orange)"; e.currentTarget.style.background = "rgba(255,193,7,0.1)"; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = "var(--border)"; e.currentTarget.style.background = "var(--bg)"; }}>→</button>
        </div>
      </div>
    </div>
  );
...
function About() {
  ...
  return (
    <section id="about" style={{ padding: "5rem 5vw", background: "var(--surface)", position: "relative", overflow: "hidden" }}>
      <style>{`.about-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 4rem; align-items: center; max-width: 1200px; margin: 0 auto; } @media (max-width: 768px) { .about-grid { grid-template-columns: 1fr; gap: 3rem; } }`}</style>
      ...
      <div className="about-grid">
        <div className="reveal-left"><AboutCarousel /></div>
        <div className="reveal-right">
          ...
          <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(2rem, 3.5vw, 3.5rem)", lineHeight: 0.95, marginBottom: "2rem" }}>Real agency.<br /><span style={{ color: "var(--orange)" }}>Real results.</span></h2>
          <p style={{ color: "var(--muted)", lineHeight: 1.8, marginBottom: "1.2rem" }}>With over 12 years of hands-on experience in digital marketing, Chirag has worked with global agency networks managing high-impact campaigns for leading brands across industries.</p>
          <p style={{ color: "var(--muted)", lineHeight: 1.8, marginBottom: "2rem" }}>Over the years, he has also helped startups set up their digital presence and launch scalable marketing campaigns from the ground up.</p>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.6rem" }}>
              {agencies.map((a) => (<span key={a} style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "6px", padding: "0.4rem 0.9rem", fontSize: "0.82rem", fontWeight: 700, letterSpacing: "0.06em", transition: "border-color 0.2s, color 0.2s" }} onMouseEnter={(e) => { e.target.style.borderColor = "var(--orange)"; e.target.style.color = "var(--orange)"; }} onMouseLeave={(e) => { e.target.style.borderColor = "var(--border)"; e.target.style.color = "var(--white)"; }}>{a}</span>))}
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <section id="contact" style={{ padding: "6rem 5vw", background: "var(--bg)", position: "relative", overflow: "hidden", textAlign: "center" }}>
      ...
      <div className="reveal" style={{ position: "relative" }}>
        ...
        <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(3rem, 6vw, 7rem)", lineHeight: 0.9, marginBottom: "2rem" }}>START YOUR<br /><span style={{ color: "var(--orange)", textShadow: "0 0 120px rgba(255,92,26,0.5)" }}>JOURNEY</span><br />TODAY</h2>
        <p style={{ color: "var(--muted)", fontSize: "1.1rem", marginBottom: "3rem", maxWidth: "500px", margin: "0 auto 3rem" }}>Join DigitalWithChirag and build the skills that top companies actually hire for.</p>
        <a href="https://wa.me/919311048234" target="_blank" rel="noreferrer"
...
function Footer() {
  return (
    <footer style={{ background: "#030305", borderTop: "1px solid var(--border)", padding: "2.5rem 5vw 5rem", display: "flex", flexDirection: "column", alignItems: "center", gap: "1.5rem" }}>
      <style>{`.footer-inner { width: 100%; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; } .footer-socials { display: flex; align-items: center; gap: 1rem; justify-content: center; } .footer-copy { font-size: 0.82rem; color: rgba(240,237,232,0.25); text-align: center; width: 100%; } @media (max-width: 600px) { .footer-inner { flex-direction: column; align-items: center; text-align: center; } }`}</style>
      <div className="footer-inner">
        <span style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "1rem", color: "var(--muted)" }}>Digital<span style={{ color: "var(--orange)" }}>With</span>Chirag</span>
        <div className="footer-socials">
          <a href="https://www.instagram.com/digitalwithchirag_" target="_blank" rel="noreferrer" style={{ color: "rgba(240,237,232,0.35)", transition: "color 0.2s" }} onMouseEnter={(e) => (e.currentTarget.style.color = "#E1306C")} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(240,237,232,0.35)")}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" /></svg>
          </a>
          <a href="https://www.facebook.com/share/18MMTKLM7r/?mibextid=wwXIfr" target="_blank" rel="noreferrer" style={{ color: "rgba(240,237,232,0.35)", transition: "color 0.2s" }} onMouseEnter={(e) => (e.currentTarget.style.color = "#1877F2")} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(240,237,232,0.35)")}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
          </a>
          <a href="https://wa.me/9311048234" target="_blank" rel="noreferrer" style={{ color: "rgba(240,237,232,0.35)", transition: "color 0.2s" }} onMouseEnter={(e) => (e.currentTarget.style.color = "#25D366")} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(240,237,232,0.35)")}>
...
export default function App() {
  ...
  return (
    <>
      ...
      {route.page !== "studio" && <Nav navigate={navigate} currentPage={route.page} />}

      {route.page === "home" && <HomePage />}
      {route.page === "blog" && <BlogList onNavigate={navigate} />}
      {route.page === "post" && <BlogPost slug={route.slug} onNavigate={navigate} />}
      {route.page === "studio" && <StudioPage />}
    </>
  );
}